...
if (!_url)     throw new Error("Falta SUPABASE_URL en variables de entorno (cargas/ocultar-historial)");
if (!_roleKey) throw new Error("Falta SUPABASE_SERVICE_ROLE_KEY en variables de entorno (cargas/ocultar-historial)");
...
export async function PATCH(req: Request) {
  ...
  // ── 4. Verificar que la carga existe y pertenece al usuario ──────────────
  ...
  // ── 5. Determinar qué campo ocultar según el rol ──────────────────────────
  ...
  if (usuario.rol === "cliente") {
    ...
    updateData.oculto_cliente = true;
  } else {
    // rol === "chofer"
    if (String(carga.chofer_id) !== String(userId)) {
      return NextResponse.json({ error: "Prohibido: este viaje no te pertenece" }, { status: 403 });
    }
    updateData.oculto_chofer = true;
  }
  ...
  if (updateError) {
    console.error("[cargas/ocultar-historial] error UPDATE:", updateError.message);
