...
if (!_url)     throw new Error("Falta SUPABASE_URL en variables de entorno (chat/marcar-leidos)");
if (!_roleKey) throw new Error("Falta SUPABASE_SERVICE_ROLE_KEY en variables de entorno (chat/marcar-leidos)");
...
export async function POST(req: Request) {
  ...
  if (!viaje_id || !tipo_chat) {
    return NextResponse.json(
      { error: "Faltan campos obligatorios: viaje_id, tipo_chat" },
      { status: 400 },
    );
  }
  ...
  if (!permitido) {
    return NextResponse.json(
      { error: "Prohibido: no tenés permiso para marcar mensajes en este canal" },
      { status: 403 },
    );
  }
  ...
  if (updateError) {
    console.error("[chat/marcar-leidos] error actualizando:", updateError.message);
    return NextResponse.json({ error: "Error interno al marcar mensajes" }, { status: 500 });
  }
