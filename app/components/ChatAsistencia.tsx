...
export default function ChatAsistencia({
  ...
  const handleEnviar = async () => {
    ...
    if (err) {
      ...
      setTexto("");
      // Scroll sólo el div interno — no window
      const el = mensajesRef.current;
      if (el) el.scrollTop = el.scrollHeight;
  ...
  // ── Área de mensajes (compartida entre modos) ────────────────────────────
  const AreaMensajes = (
    <div
      ref={mensajesRef}
    ...
    >
      {mensajes.length === 0 ? (
        <p className="text-zinc-500 text-xs text-center pt-6 select-none">
  ...
  // ── Input (flex-shrink-0 garantiza que siempre quede visible abajo) ────────
  const AreaInput = (
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // MODO INLINE
  // Compacto, autónomo — el padre no necesita proveer altura.
  // w-full max-w-md h-[420px], borde redondeado, overflow-hidden.
  // ────────────────────────────────────────────────────────────────────────────
  if (modoInline) {
    return (
      <div className="w-full max-w-md h-[420px] flex flex-col border border-zinc-700 rounded-3xl overflow-hidden bg-zinc-950">
        {AreaMensajes}
        {AreaInput}
      </div>
    );
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // MODO FLOTANTE
  // Botón fijo + ventana compacta sobre el contenido.
  // ────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <>
      ...
      {abierto && (
        <div className="fixed bottom-24 right-6 z-50 w-80 md:w-96 max-h-[70vh] bg-zinc-900 border border-zinc-700 rounded-3xl shadow-2xl flex flex-col overflow-hidden">
          {/* Header */}
          <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-zinc-800">
            ...
            <button
              ...
              className="text-zinc-500 hover:text-white w-7 h-7 flex items-center justify-center transition"
            >
              ✕
            </button>
          </div>
