...
export default function PanelChoferPage() {
  ...
  const cargarCargas = async () => {
    ...
    try {
      ...
      const cargasFiltradas = (data || []).filter((carga) => {
        ...
        return (
          String(carga.vehiculo || "").toLowerCase().trim() ===
          String(vehiculoDelChofer || "").toLowerCase().trim()
        );
      });
  ...
  const aceptarViaje = async () => {
    if (!online) {
      alert("Tenés que estar ONLINE para aceptar viajes");
      return;
    }
    ...
    if (!usuario?.id || usuario?.rol !== "chofer") {
      alert("Sesión inválida: ingresá como chofer");
      return;
    }
    ...
    if (error) {
      console.log(error);
      alert("Este viaje ya fue tomado por otro chofer");
      cargarCargas();
      return;
    }
  ...
  const BotonOnline = () => (
    <div className="w-full flex justify-center mb-6">
      <button
        ...
        className={`px-8 py-4 rounded-3xl font-black text-xl md:text-2xl shadow-2xl transition ${
          online
    ...
    <main className="min-h-screen bg-black text-white px-4 py-6 flex items-center justify-center">
      ...
      {cargando ? (
        <section className="w-full max-w-xl text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-yellow-400 animate-pulse">
            Buscando viajes...
          </h1>
        </section>
      ) : !cargaActual ? (
        <section className="w-full max-w-3xl text-center bg-zinc-900 border border-zinc-800 rounded-3xl p-8 md:p-12">
          ...
          <p className="text-green-400 font-black text-lg md:text-xl mb-4">
            Vehículo habilitado: {vehiculoChofer || "No definido"}
          </p>
          ...
          <button
            onClick={() => {
              window.location.href = "/billetera-chofer";
            }}
          ...
          <div className="mt-5 flex justify-center">
            <button
              ...
              className="bg-red-700 hover:bg-red-600 border border-red-500 hover:border-red-400 text-white font-black text-lg px-8 py-3 rounded-2xl shadow-xl transition-all duration-200"
            >
              ⛔ CERRAR SESIÓN
            </button>
          </div>
        ...
        <section className="w-full max-w-5xl bg-zinc-900 border-4 border-yellow-400 rounded-3xl p-5 md:p-8 shadow-2xl animate-pulse text-center">
          ...
          <p className="text-pink-500 font-black text-xl md:text-2xl mb-4">
            🚨 NUEVO VIAJE DISPONIBLE 🚨
          </p>

          <p className="text-green-400 font-black text-lg md:text-xl mb-6">
            Vehículo habilitado: {vehiculoChofer || "No definido"}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-lg md:text-2xl mb-8 text-left">
            ...
            <p>
              🚛 <strong>Vehículo:</strong> {cargaActual.vehiculo || "Sin dato"}
            </p>

            <p>
              📍 <strong>Distancia:</strong>{" "}
              {cargaActual.km_estimados
                ? `${cargaActual.km_estimados} km`
                : "Sin calcular"}
            </p>

            <p>
              ⚖️ <strong>Peso:</strong> {cargaActual.peso || "Sin dato"}
            </p>

            <p>
              💰 <strong>Ganancia chofer:</strong>{" "}
              ${Number(cargaActual.pago_chofer || 0).toLocaleString()}
            </p>

            <p>
              📦 <strong>Tipo:</strong> {cargaActual.tipo_carga || "Sin dato"}
            </p>

            <p className="md:col-span-2">
              📝 <strong>Detalles:</strong>{" "}
              {cargaActual.detalles || "Sin detalles"}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <button
              ...
              disabled={!online}
              className={`font-black text-2xl md:text-3xl py-6 rounded-3xl ${
                online
          ...
          <p className="text-zinc-500 text-center mt-6">
            Viaje {indice + 1} de {cargas.length}
          </p>
