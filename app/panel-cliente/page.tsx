...
export default function PanelClientePage() {
  ...
  const cargarViajes = async () => {
    ...
    console.log("[DIAG] panel-cliente historial-cliente x-user-id:", String(usuario.id));
    const res = await fetch("/api/cargas/historial-cliente", {
      headers: { "x-user-id": String(usuario.id) },
    });
    if (!res.ok) {
      const txt = await res.text();
      console.error("[DIAG] panel-cliente historial-cliente status:", res.status, "body:", txt);
      return;
    }
    const { cargas: todos = [] } = await res.json() as { cargas: any[] };
    console.log("[DIAG] panel-cliente historial-cliente cantidad:", todos.length);
    setViajes(todos);
