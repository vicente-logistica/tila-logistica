...
export default function ViajeActivoPage() {
    ...
    const watchId = navigator.geolocation.watchPosition(
      async (position) => {
        ...
        const { error } = await supabase.from("cargas").update({
          lat, lng, velocidad, velocidad_kmh: velocidad, gps_actualizado: actualizado,
        }).eq("id", viaje.id);
        if (error) { setGpsEstado("Error guardando GPS"); }
  ...
  const cargarViajeActivo = async () => {
    ...
    if (dataParadas && dataParadas.length > 0) {
  ...
  const confirmarParadaCompletada = async () => {
    ...
    if (error) { alert("Error al confirmar parada: " + error.message); setConfirmandoParada(false); return; }
  ...
  const acreditarBilletera = async (data: any) => {
    ...
    if (!choferId || !esUuidValido(choferId) || !viajeId || !montoGanancia) return;
    const { data: yaExiste } = await supabase.from("billetera_chofer").select("id").eq("viaje_id", String(viajeId)).maybeSingle();
    if (yaExiste) return;
    const { error } = await supabase.from("billetera_chofer").insert([{ chofer_id: choferId, viaje_id: String(viajeId), monto: montoGanancia }]);
    if (error) alert("Error al acreditar billetera: " + error.message);
  };
  ...
  const actualizarEstado = async (nuevoEstado: string) => {
    ...
    if (error) { alert("Error al actualizar estado"); if (nuevoEstado === "Viaje finalizado") viajeTerminado.current = false; return; }
    setViaje(data);
  ...
  // ─── Abrir Google Maps (SOLO como respaldo manual) ────────────────────────
  const abrirMapaExterno = () => {
    ...
    if (paradas.length > 0 && paradaActivaIndex < paradas.length) {
      ...
      destination = paradaActiva ? `${paradaActiva.direccion}, Argentina` : `${viaje.destino}, Argentina`;
    }
    const url = origin
  ...
  return (
    <main className="min-h-screen bg-black text-white p-4 md:p-6">
      <section className="max-w-5xl mx-auto bg-zinc-900 border-4 border-yellow-400 rounded-3xl p-5 md:p-8 shadow-2xl">
        ...
        {/* ─── MAPA PRINCIPAL — experiencia tipo Uber ──────────────────── */}
        <div className="rounded-3xl overflow-hidden border-2 border-yellow-400 mb-4">
          <MapaTILA
            lat={viaje?.lat}
            lng={viaje?.lng}
            origen={viaje.origen}
        ...
        {/* Botón Google Maps — respaldo secundario */}
        <button
          type="button"
          onClick={abrirMapaExterno}
          className="w-full bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 text-zinc-300 font-black text-sm py-3 rounded-2xl mb-6 transition"
        >
          🗺️ Abrir en Google Maps (respaldo)
        </button>
